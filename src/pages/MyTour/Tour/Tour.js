...
const Tour = ({ tour }) => {
    ...
    const handleUpadateStatus = () => {
        const url = `https://blooming-scrubland-16632.herokuapp.com/booking/${_id}`
        fetch(url, {
            method: 'PUT',
            headers: {
    ...
    const handleDeleteUser = id => {
        ...
        if (proceed) {
            const url = `https://blooming-scrubland-16632.herokuapp.com/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
    ...
    return (
        <div >
            ...
            <Col>
                <Card style={{ backgroundColor: '#c1d891c4' }} className="text-dark fw-bolder">
                    ...
                    <Card.Body className="d-md-flex justify-content-between align-items-center ">
                        ...
                        <div>
                            <div onClick={() => handleUpadateStatus(_id)}>
                                {
                                    status ? <div>
                                        <Nav.Link className="text-dark"> <i className="fas fa-circle me-2 text-warning"></i> <span>Pending</span></Nav.Link>
                                    </div>
                        ...
                        <Card.Text><button onClick={() => handleDeleteUser(_id)} className="bg-danger border-0 rounded"><i className="fas fa-trash-alt text-light"></i></button></Card.Text>
                    </Card.Body>
                </Card>
            </Col>
        </div >
